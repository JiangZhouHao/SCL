...
def train_one_step(net, loader_iter, optimizer, criterion, writter, step):
    ...
    video_scores, contrast_pairs, _, _ ,video_scores_supp,cas_supp,fore_weights= net(data)

    cost, loss = criterion(video_scores, label, contrast_pairs,video_scores_supp,fore_weights)
...
def test_all(net, cfg, test_loader, test_info, step, writter=None, model_file=None):
    ...
    for data, label, _, vid, vid_num_seg in test_loader:
        ...
        video_scores, _, actionness, cas,video_scores_supp,cas_supp,fore_weights = net(data)
